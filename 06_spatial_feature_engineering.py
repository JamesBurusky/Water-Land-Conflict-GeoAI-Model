...
EXCLUDE_DATE_PRECISION_SUSPECT = False  # see src/conflict_cleaning.py's parse_dates() --
    # 39.1% of records are dated exactly Jan 1, disproportionately consistent with
    # year-only source precision defaulted to Jan 1 rather than a genuine date. Since
    # conflict_onset and conflict_persistence are both computed at MONTH granularity,
    # a mis-dated record pollutes two months at once (false signal where it lands,
    # missing signal where it belongs). Excluding these from the panel's dynamic
    # features is the applied fix; set False to reproduce the pre-fix panel for
    # direct before/after comparison (do this deliberately, not by accident, since
    # the two panels are NOT interchangeable for reporting purposes).
...
_geocoded_cols_to_save = ["Record_ID", "geo_county", "geo_subcounty"]
if "date_precision_suspect" in conflict_geocoded.columns:
    _geocoded_cols_to_save.append("date_precision_suspect")
conflict_geocoded[_geocoded_cols_to_save].to_csv(
...
if EXCLUDE_DATE_PRECISION_SUSPECT and "date_precision_suspect" in conflict_geocoded.columns:
    n_suspect = conflict_geocoded["date_precision_suspect"].fillna(False).sum()
    conflict_geocoded_for_features = conflict_geocoded[~conflict_geocoded["date_precision_suspect"].fillna(False)].copy()
    print(f"  EXCLUDE_DATE_PRECISION_SUSPECT=True: excluding {n_suspect}/{len(conflict_geocoded)} "
          f"records dated exactly Jan 1 (suspected year-only precision) from conflict_onset, "
          f"conflict_persistence, and conflict_severity_weighted -- {len(conflict_geocoded_for_features)} "
          f"records remain for these month-level features. This does NOT affect any other analysis "
          f"in this pipeline; only the monthly panel features built below are affected.")
else:
    conflict_geocoded_for_features = conflict_geocoded
log("STAGE 2/5: DONE.\n")
...
grid = add_conflict_features(grid, conflict_geocoded_for_features, half_life_days=HALF_LIFE_DAYS,
