...
# SHAP is run for ONE horizon (the primary/default forecast window) --
# not looped across all eight like 10_ml_modeling.py and
# 12_conflict_risk_mapping.py, to keep this step's output focused on a
# single, clearly-stated model rather than multiplying the number of
# SHAP plots by eight. Change SHAP_HORIZON_MONTHS if a different
# horizon is your primary one for the thesis narrative -- it MUST be
# one of the values in 10_ml_modeling.py's HORIZONS_MONTHS list
# ([3, 6, 9, 12, 15, 18, 21, 24] as of this pipeline version), since
# that's what determines which horizon_<N>month folders actually exist
# under outputs/10_ml_modeling/. (Previously hardcoded to 1, left over
# from before HORIZONS_MONTHS was widened -- "1" was never re-added to
# that list, so outputs/10_ml_modeling/horizon_1month/ stopped
# existing and this script failed with a bare "not found" until fixed.)
SHAP_HORIZON_MONTHS = 3
...
if not MODEL_PATH.exists():
    step10_dir = MODEL_DIR.parent
    available = sorted(
        p.name.replace("horizon_", "").replace("month", "")
        for p in step10_dir.glob("horizon_*month") if p.is_dir()
    ) if step10_dir.exists() else []
    if available:
        print(f"  {MODEL_PATH} not found. 10_ml_modeling.py HAS run, but not for "
              f"SHAP_HORIZON_MONTHS={SHAP_HORIZON_MONTHS} months specifically -- it only trained "
              f"these horizons: {', '.join(available)} month(s). Set SHAP_HORIZON_MONTHS above to "
              f"one of those values, or add {SHAP_HORIZON_MONTHS} to HORIZONS_MONTHS in "
              f"10_ml_modeling.py and re-run it.")
    else:
