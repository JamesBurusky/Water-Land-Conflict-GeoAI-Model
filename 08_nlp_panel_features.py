...
EXCLUDE_DATE_PRECISION_SUSPECT = False  # MUST match 06_spatial_feature_engineering.py's
                                        # setting -- see that script's comment for the reasoning
...
# date_precision_suspect exists on BOTH sides of this merge (nlp already
# carries it forward from 02 via 03/04/07; geocoded also has it, saved
# there by 06) -- pandas' default merge suffixes turn the single column
# into date_precision_suspect_x/_y rather than raising an error, so a
# plain "date_precision_suspect" in nlp_geocoded.columns check silently
# evaluates False and the exclusion below would never fire. Handle both
# the clean and suffixed cases explicitly instead of assuming one.
_date_precision_col = next(
    (c for c in ["date_precision_suspect", "date_precision_suspect_x", "date_precision_suspect_y"]
     if c in nlp_geocoded.columns), None
)
if EXCLUDE_DATE_PRECISION_SUSPECT and _date_precision_col is not None:
    n_suspect = nlp_geocoded[_date_precision_col].fillna(False).sum()
    nlp_geocoded_for_features = nlp_geocoded[~nlp_geocoded[_date_precision_col].fillna(False)].copy()
    print(f"  EXCLUDE_DATE_PRECISION_SUSPECT=True: excluding {n_suspect}/{len(nlp_geocoded)} "
          f"records dated exactly Jan 1 (suspected year-only precision) from decayed_sentiment, "
          f"dominant_topic_id, and topic_diversity -- consistent with the same exclusion applied "
          f"to conflict_onset/persistence in 06_spatial_feature_engineering.py.")
else:
    nlp_geocoded_for_features = nlp_geocoded

panel = add_nlp_features(panel, nlp_geocoded_for_features, half_life_days=HALF_LIFE_DAYS,
