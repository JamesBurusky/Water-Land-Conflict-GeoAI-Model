...
# Columns most useful for manually re-checking a date against its
# original source -- adjust this list if your CSV's column names
# differ, but these match Final_Main_kenya_land_water_conflicts.csv
# as used throughout the rest of this project.
REVIEW_COLUMNS = [
...
def main():
    ...
    review_cols = [c for c in REVIEW_COLUMNS if c in df.columns]
    missing_cols = [c for c in REVIEW_COLUMNS if c not in df.columns]
    if missing_cols:
        print(f"  NOTE: expected column(s) not found in this CSV, skipped: {missing_cols}")

    suspect_df = df.loc[date_suspect, review_cols].sort_values(
        by=[c for c in ["Source_Name", "Source_URL"] if c in review_cols]
    )
